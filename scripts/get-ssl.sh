...
# Использование: ./get-ssl.sh [DOMAIN] [EMAIL]
...
if ! systemctl is-active --quiet nginx; then
    error "Nginx не запущен. Запустите сначала setup-nginx.sh"
    exit 1
fi
...
success "SSL сертификат успешно настроен!"
log "Ваш сайт теперь доступен по адресу: https://$DOMAIN"
log "Следующий шаг: запустите ./scripts/deploy-with-nginx.sh для деплоя приложения" 