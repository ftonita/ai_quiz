...
# Скрипт для настройки Nginx на сервере для домена v386879.hosted-by-vdsina.com
# Использование: ./setup-nginx-vdsina.sh
...
success "Nginx успешно настроен для домена $DOMAIN!"
log "Следующие шаги:"
log "1. Убедитесь, что DNS записи для домена $DOMAIN указывают на этот сервер"
log "2. Запустите скрипт получения SSL сертификата: ./scripts/get-ssl-vdsina.sh"
log "3. После получения сертификата запустите: ./scripts/deploy-with-nginx-vdsina.sh"
