...
# Скрипт для деплоя приложения с Nginx для домена v386879.hosted-by-vdsina.com
# Использование: ./deploy-with-nginx-vdsina.sh
...
if [ ! -f "/etc/nginx/sites-available/$DOMAIN" ]; then
    ...
    log "Сначала запустите: ./scripts/setup-nginx-vdsina.sh"
    exit 1
fi
...
if [ ! -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]; then
    ...
    log "Сначала запустите: ./scripts/get-ssl-vdsina.sh your-email@example.com"
    exit 1
fi
...
success "Деплой с Nginx завершен для домена $DOMAIN!"
